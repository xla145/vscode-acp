...
async function focusChatPanel(): Promise<void> {
  ...
  await vscode.commands.executeCommand('fastopc-chat.focus');
}
...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const quchiAuthService = new QuchiAuthService(sessionManager);

  // --- UI ---
  const workspaceCwd = () => vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
  const sessionTreeProvider = new SessionTreeProvider(sessionManager, historyStore, workspaceCwd);
  const treeView = vscode.window.createTreeView('fastopc-sessions', {
  ...
  const connectAgentCmd = vscode.commands.registerCommand('acp.connectAgent', async () => {
    ...
    try {
      if (!quchiAuthService.isAuthenticated()) {
        await focusChatPanel();
        chatWebviewProvider.notifyAuthChanged();
  ...
  // Open Chat (right-side auxiliary bar)
  const openChatCmd = vscode.commands.registerCommand('acp.openChat', () => focusChatPanel());

  // Send Prompt (from keybinding — just focus chat)
  const sendPromptCmd = vscode.commands.registerCommand('acp.sendPrompt', () => focusChatPanel());
  ...
  const openSessionCmd = vscode.commands.registerCommand('acp.openSession', async (arg?: any) => {
    ...
    try {
      ...
      if (caps?.load) {
        ...
        // Agent doesn't support session load/resume.
        // Restore locally-persisted history so the user can view the old
        // conversation, then reconnect so they can continue chatting.
        log(`Agent "${agentName}" does not support session load/resume, restoring local history.`);
        const entry = historyStore.get(agentName, sessionId);
        chatWebviewProvider.notifyRestoreMessages(
          sessionId,
          agentName,
  ...
  const signOutCmd = vscode.commands.registerCommand('acp.signOut', async () => {
    await quchiAuthService.signOut();
    chatWebviewProvider.notifyAuthChanged();
    vscode.window.showInformationMessage('已退出 Quchi 登录。');
  });
  ...
  // Open chat panel on startup; agent connection is allowed only after Quchi authorization.
  setTimeout(() => {
    void focusChatPanel();
  }, 500);
}
